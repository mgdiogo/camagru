...
class TokenModel extends Model {
	...
	public function getPasswordUpdateToken($token) {
		$this->db->query("
			SELECT 
				users.id,
				users.verified,
	...
	public function getTempPassword($token) {
		$this->db->query('SELECT user_id, temp_password FROM user_tokens WHERE token = :token AND type = :type');
		$this->db->bind('token',$token);
		$this->db->bind('type','password_update');
		$this->db->execute();
	...
	public function setTempPassword($password, $id) {
		try {
			$this->db->query('UPDATE user_tokens SET temp_password = :temp_password WHERE user_id = :user_id AND type = :type');
			$this->db->bind('temp_password', $password);
			$this->db->bind('user_id', $id);
			$this->db->bind('type', 'password_update');
			$this->db->execute();
		} catch (PDOException $e) {
			error_log("Error setting temp password: " . $e->getMessage());
		}
	}
	...
	public function generateUpdatePasswordToken($id) {
